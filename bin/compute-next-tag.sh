...
#
# ---------------------------------------------------------------------------
# AnonymousOverflow has no upstream version - read this before changing anything
# ---------------------------------------------------------------------------
#
# Most roles in the fleet name their tags after the version of the software
# they install. This one cannot: AnonymousOverflow publishes no versioned
# container images.
#
# - github.com/httpjamesm/AnonymousOverflow carries git tags (v1.1 through
#   v1.14.0 at the time of writing), but its release pipeline does not turn
#   them into image tags
# - ghcr.io/httpjamesm/anonymousoverflow carries exactly one tag, `release`.
#   Its `.github/workflows/docker-image.yml` runs on pushes to the `release`
#   branch and hands `docker/metadata-action` no `tags` input at all, so the
#   only tag it ever produces is the branch name
#
# So `anonymousoverflow_version` in defaults/main.yml is the literal `release`,
# there is nothing for Renovate to bump, and there is no version to name a tag
# after. The tags this repository has always used encode that absence as the
# placeholder version `2024.8.23` - the date the `release` image was built,
# which is where it has stood ever since - with the release counter carrying
# all of the information: `v2024.8.23-0` through `v2024.8.23-15` at the time of
# writing.
#
# This script reproduces that naming rather than inventing a new one, by
# translating the `release` sentinel into `2024.8.23`. It still reads the
# version out of defaults/main.yml, so that the day AnonymousOverflow starts
# publishing versioned images and this role pins one, tagging follows along on
# its own: pinning `anonymousoverflow_version: 1.14.0` starts a `v1.14.0-N`
...
# - otherwise the counter is incremented (`v2024.8.23-16`), but only if
...
# `anonymousoverflow_container_image_self_build_repo_version`) can be picked up
# instead.
#
# `anonymousoverflow_version` is a literal value. Deriving the tag from a
# variable whose value is a Jinja expression would put that expression into the
# tag itself - a mistake that has produced tags such as `v{{-0` elsewhere in the
# fleet.
version="$(sed -nE 's|^anonymousoverflow_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the AnonymousOverflow version from $defaults_path"
	exit 1
fi
...
# already past that boundary, at `v2024.8.23-15`.
